...
CUSTOM_REWARD_FUNCTION_PATH=$(Y custom_reward_function_path src/opsd_rlcsd/verl_reward.py)
...
MAIN=src.opsd_rlcsd.self_distill_main
case "$METHOD" in
...
# Precompute values that used inline `case` inside $(...). macOS bash 3.2 fails
# to parse a case-statement nested in command substitution, so resolve them into
# plain variables here (portable across all bash versions).
if [ -n "$KL_LOSS_COEF" ] && [ "$KL_LOSS_COEF" != "None" ]; then
    KL_LOSS_COEF_VALUE="$KL_LOSS_COEF"
else
    case "$METHOD" in
        rlsd|sdpo|srpo|opsd|opd) KL_LOSS_COEF_VALUE="0" ;;
        *)                       KL_LOSS_COEF_VALUE="0.001" ;;
    esac
fi
case "$METHOD" in
    grpo|opd) POLICY_LOSS_MODE="vanilla" ;;
    *)        POLICY_LOSS_MODE="$METHOD" ;;
esac
...
python3 -m $MAIN \
    ...
    actor_rollout_ref.actor.kl_loss_coef=${KL_LOSS_COEF_VALUE} \
