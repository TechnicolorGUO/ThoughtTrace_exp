...
try:
    from src.opsd_format import build_rollout_messages, strip_legacy_math_prompt
except ImportError:
    build_rollout_messages = None
    strip_legacy_math_prompt = None

OPSD_PROMPT_DATA_SOURCES = {
    "dapo_math_17k",
...
def _opsd_prompt_problem_from_messages(messages: list[dict]) -> str:
    parts = []
    ...
    merged = "\n".join(parts).strip()
    if strip_legacy_math_prompt is None:
        return merged
    return strip_legacy_math_prompt(merged)
...
class RLHFDataset(Dataset):
    ...
    def _build_messages(self, example: dict):
        ...
        if build_rollout_messages is not None and data_source in OPSD_PROMPT_DATA_SOURCES:
            extra_info = example.get("extra_info") or {}
            problem = extra_info.get("problem", "") if isinstance(extra_info, dict) else ""
            if not problem:
                problem = _opsd_prompt_problem_from_messages(messages)
            messages = build_rollout_messages(problem, data_source=data_source)
        return messages
