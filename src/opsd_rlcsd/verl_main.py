...
from src.opsd_rlcsd.data_utils import prepare_training_data, prepare_eval_data
from src.opsd_rlcsd.prompts import (
    STUDENT_SYSTEM_MESSAGE,
    TEACHER_SYSTEM_MESSAGE,
    build_student_messages,
    build_teacher_messages,
)
from src.opsd_rlcsd.opsd_format import _answer_instruction
from src.opsd_rlcsd.losses import (
    generalized_jsd_loss, sdpo_loss, rlsd_loss,
    compute_entropy, compute_grpo_advantages, compute_rollout_is_weights,
)
from src.opsd_rlcsd.models import TeacherContext, EMATeacher
from src.opsd_rlcsd.verl_reward import compute_reward
...
class VerlConfig:
    ...
    experiment_name: str = ""
    data_dir: str = "./data"
    train_dataset: str = "data/processed/user_sim_opsd_train.jsonl"
    val_dataset: str = "data/processed/user_sim_opsd_test.jsonl"
...
def build_student_input(tokenizer, problem, enable_thinking, thinking=False):
    ...
    return tokenizer.apply_chat_template(
        build_student_messages(problem),
        tokenize=False,
...
def normalize_privileged_text_mode(mode):
    ...
    if normalized in {"solution", "thought", "private_thought", "solution_answer"}:
        return "solution"
    if normalized not in {"solution"}:
        raise ValueError(
            f"Unsupported privileged_text_mode={mode!r}. "
            "Supported values for ThoughtTrace: solution, thought, private_thought."
        )
    return "solution"
...
def build_teacher_input(tokenizer, problem, answer, solution, privileged_text_mode, enable_thinking, thinking=False):
    del answer, thinking
    normalize_privileged_text_mode(privileged_text_mode)
    kwargs = {}
    if enable_thinking is not None:
        kwargs["enable_thinking"] = bool(enable_thinking)
    return tokenizer.apply_chat_template(
        build_teacher_messages(problem, solution),
        tokenize=False,
        add_generation_prompt=True,
        **kwargs,
    )
...
def load_flat_yaml(path: str) -> dict:
    data = {}
    with open(path, encoding="utf-8") as f:
        for line_number, raw_line in enumerate(f, start=1):
            ...
            if not line:
                continue
            if ":" not in line:
                raise ValueError(f"Unsupported config line at {path}:{line_number}: {raw_line.rstrip()}")
...
class VerlTrainer:
    ...
    def train(self):
        ...
        eval_data = None
        if self.is_main:
            eval_data = prepare_eval_data(cfg.data_dir, cfg.val_dataset)
            ...
            for batch_idx, batch in enumerate(dataloader):
                ...
                if missing_indices:
                    ...
                    if len(missing_indices) > 8:
                        preview = f"{preview}, ..."
                    raise ValueError(
                        "ThoughtTrace OPSD requires every sample to have non-empty private thought "
    ...
    def _evaluate_vllm(self, llm, tokenizer, eval_data):
        ...
        for name, data in eval_data.items():
            ...
            lengths = [len(str(response).split()) for response in responses]
            results[f"eval/{name}/num_examples"] = len(responses)
            results[f"eval/{name}/avg_response_words"] = sum(lengths) / max(len(lengths), 1)
        return results
