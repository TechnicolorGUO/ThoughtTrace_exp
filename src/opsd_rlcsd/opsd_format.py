...
def normalize_privileged_text_mode(mode: str) -> str:
    ...
    if normalized in {"solution", "solution_only", "thought", "thought_only"}:
        normalized = "solution_only"
    if normalized not in {"solution_answer", "answer_only", "solution_only"}:
        raise ValueError(
            f"Unsupported privileged_text_mode={mode!r}. "
            "Supported values: solution_answer (alias: solution+answer), answer_only, "
            "solution_only (alias: thought_only)."
        )
    return normalized
...
def build_teacher_privileged_text(answer: str, solution: str, mode: str) -> tuple[str, str]:
    ...
    if mode == "solution_only":
        # Privileged text is the solution/thought ALONE. The ground-truth answer
        # is deliberately excluded so it never leaks into the teacher context
        # (used for user-simulation / next-message distillation).
        if not solution:
            raise ValueError("privileged_text_mode=solution_only requires a non-empty GT solution")
...
def build_teacher_messages(
    ...
    if effective_mode == "solution_only":
        # Dialogue / user-simulation teacher prompt. `problem` already carries the
        # full shared context (conversation history + assistant's latest reply),
        # so it is used verbatim -- no math stripping, no \boxed{} instruction.
        # The privileged text is the user's private thought and nothing else.
        del thinking
        user_content = (
            f"{problem}\n\n"
            "[The user's current thoughts]\n"
            f"{privileged_text}\n\n"
            "Based on these thoughts, write what the user would say next."
        )
