...
class CustomScriptArguments(ScriptArguments):
    ...
    dataset_name: str = field(
        ...
        metadata={
            "help": "Dataset to train on. Either an HF hub repo id, or a path to a local file "
            "(.jsonl/.json/.parquet/.csv) or directory. The dataset must expose the columns named by "
            "`problem_column` and `solution_column` (renamed to `problem`/`solution` automatically)."
        },
    )
    dataset_split: str = field(
        default="train",
        metadata={"help": "Which split to use as the training set."},
    )
    dataset_config_name: str = field(
        default=None,
        metadata={"help": "Optional HF dataset config/subset name passed to load_dataset(name=...)."},
    )
    problem_column: str = field(
        default="problem",
        metadata={
            "help": "Column holding the student-visible prompt (the instruction). Renamed to `problem` "
            "internally. OPSD conditions the student only on this text."
        },
    )
    solution_column: str = field(
        default="solution",
        metadata={
            "help": "Column holding the privileged reference (the target/answer the teacher additionally "
            "sees). Renamed to `solution` internally. For plain instruction data, map this to the response."
        },
    )
...
if __name__ == "__main__":
    ...
    # Dataset
    ################
    # Load the dataset with privileged references
    ################
    # OPSD needs two columns: `problem` (student-visible prompt) and `solution`
    # (privileged reference the teacher additionally sees). The source columns are
    # configurable via --problem_column / --solution_column and renamed here.
    ################
    # Training
    ################
    ...
    local_exts = (".jsonl", ".json", ".parquet", ".csv")
    if os.path.isfile(script_args.dataset_name) and script_args.dataset_name.endswith(local_exts):
        ext = os.path.splitext(script_args.dataset_name)[1].lstrip(".")
        loader = "json" if ext == "jsonl" else ext
        dataset = load_dataset(loader, data_files=script_args.dataset_name)
    else:
    ...
    rename_map = {}
    if script_args.problem_column != "problem":
        rename_map[script_args.problem_column] = "problem"
    if script_args.solution_column != "solution":
        rename_map[script_args.solution_column] = "solution"
    if rename_map:
        train_dataset = train_dataset.rename_columns(rename_map)

    missing = [c for c in ("problem", "solution") if c not in train_dataset.column_names]
    if missing:
        raise ValueError(
            f"Dataset is missing required column(s) {missing} after renaming. "
            f"Available columns: {train_dataset.column_names}. "
            f"Set --problem_column / --solution_column to map your fields."
        )
